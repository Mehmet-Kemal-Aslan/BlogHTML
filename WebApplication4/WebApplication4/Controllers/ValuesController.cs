...
namespace test
{
    ...
    public class DtoBlogDeleteComment
    {
        public int CommentId { get; set; }
    }

    public class DtoBlogSetName
    {
        public string NewName { get; set; }
        public int UserId { get; set; }
    }

    public class DtoBlogSetPassword
    {
    ...
    namespace test.Controllers
    {
...

        {
            ...
            [HttpGet]
            public object Articles3(int commentId)
            {
                using (var conn = new SqlConnection(connStr))
                {
                    conn.Open();
            ...
            [Route("api/SetName")]
            [HttpPost]
            public object Name([FromBody] DtoBlogSetName followerName)
            {
                using (var conn = new SqlConnection(connStr))
                {
                    conn.Open();

                    var result = conn.Execute("UPDATE Followers SET Username = '" + followerName.NewName + "' WHERE Id= '" + followerName.UserId + "'");
            ...
            [HttpPost]
            public object Password([FromBody] DtoBlogSetPassword followerPassword)
            {
                using (var conn = new SqlConnection(connStr))
                {
                    conn.Open();

                    var result = conn.Execute("UPDATE Followers SET Password = '" + followerPassword.NewPassword + "' WHERE Id= '" + followerPassword.UserId + "'");
